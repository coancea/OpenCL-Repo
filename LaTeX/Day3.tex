...
\begin{itemize}
    ...
    \item[(III)] Do all the rest in one kernel:
        \begin{itemize}
        \begin{scriptsize}
            \item[(1)] Copy {\tt ELEMS\_PER\_THREAD} from global to local memory, coalesced (ToDo).\smallskip
            \item[(2)] Save them in per-thread register memory ({\tt chunk}) +\\
                        Reduce sequentially each thread's {\tt ELEMS\_PER\_THREAD}
                        elems in {\tt tf}; (ToDo).\smallskip
            \item[(3)] Perform workgroup-level scan on the {\tt tf} of each thread (DONE).\smallskip
            \item[(4)] The number of elements that succeed/fail per workgroup 
                        ({\tt num\_ts/num\_fs}) is the last scanned element (DONE).\smallskip

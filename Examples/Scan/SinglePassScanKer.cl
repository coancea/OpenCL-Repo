...
__kernel void singlePassScanGroupVirtKer (
        uint32_t            N,
        __global ElTp*      d_inp,      // read-only,  [N]
        __global ElTp*      d_out,      // write-only, [N]
        volatile __global int32_t*   global_id,  // read-write, [1]
    ...
    const int32_t num_virt_iters = CEIL_DIV(num_virtgroups - get_group_id(0),
                                            get_num_groups(0));
    for (int32_t _ = 0; _ < num_virt_iters; _++) {
    if (tid == 0) {
        int32_t id  = atomic_add(&global_id[0], 1);
        statusflgs[id] = STATUS_X;
        block_id[0] = id;
...
__kernel void singlePassSgmScanGroupVirtKer (
        uint32_t            N,
        __global uint8_t*   d_flg,      // read-only,  [N]
        __global ElTp*      d_inp,      // read-only,  [N]
        __global ElTp*      d_out,      // write-only, [N]
